      ...
      INTEGER INDXX,SAMTYPE,INDX,IOSS,LENGTH,STATUS,NARGS
      CHARACTER(120)FLAG,VALOR,FREQFILE,MODLFILE !WARNING: FIXED LENGTH
      LOGICAL ISFREQFILE,ISMODLFILE
      INTEGER TO_STD,NKS                               
      ! Initializing first to invalid or default values 
      FMIN=-1;FMAX=-1;G_NX=-1;NCAPAS=-1;NMODESRL=0;TO_STD=0;
      ...
      ENDIF      
      INDXX=0      
      DO WHILE(INDXX<NARGS)
        INDXX=INDXX+1
        CALL GET_COMMAND_ARGUMENT(INDXX,FLAG,LENGTH,STATUS)
        ! CALL LCASE@(FLAG)! fails in gfortran        
        IF(FLAG(1:1)=='-')THEN
...
!         To build the vector of frequencies           
          CASE('fmin')
            INDXX=INDXX+1
            CALL GET_COMMAND_ARGUMENT(INDXX,VALOR,LENGTH,STATUS)
            READ(VALOR,*)FMIN  
          CASE('fmax')
            INDXX=INDXX+1
            CALL GET_COMMAND_ARGUMENT(INDXX,VALOR,LENGTH,STATUS)
          ...
          CASE('ff')
            INDXX=INDXX+1
            CALL GET_COMMAND_ARGUMENT(INDXX,FREQFILE,LENGTH,STATUS)
            ISFREQFILE=.TRUE.  
...
!         Reading and building the model from model file
          CASE('f')
            INDXX=INDXX+1
            CALL GET_COMMAND_ARGUMENT(INDXX,MODLFILE,LENGTH,STATUS) 
